  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const [dashRes, agentsRes] = await Promise.all([
        api.get(`/supervisor/dashboard?t=${Date.now()}`),
        api.get('/supervisor/agents')
      ]);
      
      const fetchedTickets = dashRes.data.tickets || [];
      const fetchedAgents = agentsRes.data || dashRes.data.agents || [];
      
      setRecentTickets(fetchedTickets);
      setRecentLogs(dashRes.data.activity_logs || []);
      setAgents(fetchedAgents); 
      
      setStats({
        openTickets: fetchedTickets.filter(t => t.statusid === 1).length,
        inProgress: fetchedTickets.filter(t => t.statusid === 2).length,
        resolvedToday: fetchedTickets.filter(t => t.statusid === 3).length,
        activeAgents: fetchedAgents.length || 0
      });
        ...
        <aside className="desktop-sidebar" style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 252, background: '#fff', borderRight: '1px solid var(--border)', zIndex: 20, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '4px 0 24px rgba(15,23,42,0.04)' }}>
          <div>
            <div style={{ padding: '22px 22px 18px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 600, color: 'var(--accent)', letterSpacing: '.14em', textTransform: 'uppercase' }}>Supervisor Ops</div>
                </div>
              </div>
            </div>
            ...
            <nav style={{ padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
              {[
                { emoji: '◧', label: 'Department Overview', id: 'overview' },
                { emoji: '⊟', label: 'Category Tickets', id: 'tickets' },
                { emoji: '◎', label: 'My Agents', id: 'team' },
        ...
        <main className="main-content" style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1, marginLeft: 252 }}>
          ...
          <header className="dash-header" style={{ position: 'sticky', top: 0, zIndex: 15, background: 'rgba(243,249,255,.88)', backdropFilter: 'blur(16px)', borderBottom: '1px solid var(--border)', padding: '12px 30px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 0 var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
              ...
              <select value={filterAssignee} onChange={(e) => setFilterAssignee(e.target.value)} style={{ padding: '8px 12px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 12, outline: 'none', background: '#fff', cursor: 'pointer' }}>
          ...
          <div className="dashboard-pad" style={{ padding: '28px 30px', flex: 1, maxWidth: 1400, width: '100%', margin: '0 auto' }}>
            ...
            {activeTab === 'overview' && (
              <div className="fade-up">
                ...
                <div className="charts-grid" style={{ marginBottom: 22, display: 'grid', gridTemplateColumns: '1fr', gap: 20 }}>
                  {/* --- RECHARTS: Incident Severity Density (Bar Chart) spans full width --- */}
                  <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 18, padding: 24, boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ fontSize: 15, fontWeight: 800, color: 'var(--txt-primary)', margin: '0 0 16px' }}>Incident Severity Density</h3>
                ...
                <div className="charts-grid" style={{ marginBottom: 22, display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20 }}>
                  <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 18, boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
                    <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <button onClick={() => setActiveTab('tickets')} style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>View All →</button>
                    </div>
                          ...
                          {filteredTickets.slice(0, 5).map((ticket) => {
                            ...
                            return (
                              <tr key={ticket.id} onClick={() => setSelectedTicket(ticket)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                                <td style={{ padding: '14px 20px', fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: 'var(--accent)' }}>{ticket.referenceno || `TKT-${ticket.id}`}</td>
                      ...
                      {recentLogs.slice(0, 10).map((log, index) => {
                        ...
                        return (
                          <div key={index} style={{ position: 'relative', paddingLeft: 16, borderLeft: '2px solid var(--border)', paddingBottom: 16 }}>
                            <div style={{ position: 'absolute', left: -5, top: 4, width: 8, height: 8, borderRadius: '50%', background: 'var(--accent)', border: '2px solid #fff' }} />
            ...
            {/* ========================================= */}
            {/* TAB 2: TICKET DESK (Full Table) */}
            {/* ========================================= */}
            {activeTab === 'tickets' && (
              <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <div style={{ marginBottom: 24 }}>
                  <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--txt-primary)', letterSpacing: '-.5px', margin: '0 0 4px' }}>Ticket Desk</h1>
                  <p style={{ fontSize: 14, color: 'var(--txt-muted)', margin: 0 }}>Showing {filteredTickets.length} active tickets in your category.</p>
                </div>
                ...
                <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 18, boxShadow: 'var(--shadow-sm)', overflow: 'hidden', flex: 1 }}>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--border)', background: '#fafbfe' }}>
                          {['Reference', 'Title', 'Priority', 'Status', 'Assigned To', 'Created At'].map((h, i) => (
                      ...
                      <tbody>
                        {filteredTickets.map((ticket) => {
                          ...
                          return (
                            <tr key={ticket.id} onClick={() => setSelectedTicket(ticket)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                              <td style={{ padding: '16px 20px', fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: 'var(--accent)', whiteSpace: 'nowrap' }}>{ticket.referenceno || `TKT-${ticket.id}`}</td>
                              ...
                              <td style={{ padding: '16px 20px', whiteSpace: 'nowrap' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                  {ticket.agent_name ? (
                                    <>
                                      <div style={{ width: 16, height: 16, borderRadius: '50%', background: 'var(--accent-light)', color: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800 }}>
                                        {ticket.agent_name.charAt(0)}
                                      </div>
                                      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--txt-primary)' }}>{ticket.agent_name}</span>
                                    </>
                                  ) : (
                                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--txt-muted)' }}>Unassigned</span>
                                  )}
                                </div>
                              </td>
                              <td style={{ padding: '16px 20px', whiteSpace: 'nowrap' }}>
            ...
            {activeTab === 'team' && (
              <div className="fade-up">
                <div style={{ marginBottom: 24 }}>
                  <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--txt-primary)', letterSpacing: '-.5px', margin: '0 0 4px' }}>Agent Roster</h1>
                  <p style={{ fontSize: 14, color: 'var(--txt-muted)', margin: 0 }}>Workload distribution across your managed team.</p>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
                  {(agents.length > 0 ? agents : [
                    { id: 1, fullname: 'Agent Dummy 1', load: 8, capacity: 15 },
                    { id: 2, fullname: 'Agent Dummy 2', load: 14, capacity: 15 }
                  ]).map((agent, i) => {
                    const agentName = agent.fullname || agent.name || 'Unknown Agent';
                    const agentLoad = agent.load || agent.active_tickets_count || 0;
                    const agentCapacity = agent.capacity || 15;
                    const loadPercentage = (agentLoad / agentCapacity) * 100;
                    const loadColor = loadPercentage > 85 ? 'var(--rose)' : loadPercentage > 50 ? 'var(--amber)' : 'var(--emerald)';

                    return (
                      <div key={i} style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 16, padding: 24, boxShadow: 'var(--shadow-sm)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                          <div style={{ width: 50, height: 50, borderRadius: 14, background: 'linear-gradient(135deg, var(--accent) 0%, var(--accent-mid) 100%)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, boxShadow: '0 6px 16px rgba(79,70,229,.2)' }}>
                            {agentName.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <h3 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 800, color: 'var(--txt-primary)' }}>{agentName}</h3>
                            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: 'var(--accent)' }}>Level 1 Support</p>
                          </div>
                        </div>

                        <div style={{ background: 'var(--bg-2)', borderRadius: 12, padding: 16 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--txt-secondary)' }}>Active Workload</span>
                            <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--txt-primary)' }}>{agentLoad} / {agentCapacity}</span>
                          </div>
                          <div style={{ width: '100%', height: 8, background: 'var(--border)', borderRadius: 4, overflow: 'hidden' }}>
                            <div style={{ width: `${loadPercentage}%`, height: '100%', background: loadColor, borderRadius: 4, transition: 'width 1s ease-out' }} />
                          </div>
                        </div>
                      </div>
                    );
